...
const T={bg:"#f8f7f4",white:"#fff",teal:"#0d6b6e",tealL:"#1ab5a5",tealBg:"#e1f5ee",gold:"#c8a84e",goldBg:"#fdf6e3",goldD:"#a08030",purple:"#5a3d6e",txt:"#1a1a1a",txt2:"#555",mute:"#999",light:"#bbb",border:"#e8e6e0",ok:"#1a7d42",okBg:"#e1f9ec",err:"#c0392b",errBg:"#fde8e8",warn:"#854f0b",warnBg:"#fef3e2",
...
export default function App(){
  ...
  const[quizzes,setQuizzes]=useState([]);const[articles,setArticles]=useState([]);const[resources,setResources]=useState([]);const[videos,setVideos]=useState([]);const[forumPosts,setForumPosts]=useState([]);const[allUsers,setAllUsers]=useState([]);
  ...
  const[pf,setPf]=useState({degree:"",clinic:"",address:""});const[edForm,setEdForm]=useState(null);const[newForum,setNewForum]=useState(false);const[fpT,setFpT]=useState("");const[fpB,setFpB]=useState("");const[fpC,setFpC]=useState(TOPICS[0]);

  const sh=m=>setToast(m);const go=p=>{setPg(p);setSelA(null);setSelV(null);setEdForm(null)};
  useEffect(()=>{if(toast){const t=setTimeout(()=>setToast(null),3000);return()=>clearTimeout(t)}},[toast]);

  const loadData=useCallback(async()=>{const[q,a,r,v,f,u]=await Promise.all([fbGetAll("quizzes","date","desc"),fbGetAll("articles","date","desc"),fbGetAll("resources","order","asc"),fbGetAll("videos","order","asc"),fbGetAll("forum","createdAt","desc"),fbGetAll("users","joined","desc")]);setQuizzes(q);setArticles(a);setResources(r);setVideos(v);setForumPosts(f);setAllUsers(u)},[]);
  ...
  const doEmailSignup=async()=>{setAuthErr("");if(!authName.trim())return setAuthErr("Enter name");if(!authEmail.trim())return setAuthErr("Enter email");if(authPass.length<6)return setAuthErr("Password: 6+ chars");setAuthBusy(true);try{const c=await createUserWithEmailAndPassword(auth,authEmail,authPass);await updateProfile(c.user,{displayName:authName})}catch(e){setAuthErr(e.code==="auth/email-already-in-use"?"Email registered":e.code==="auth/invalid-email"?"Invalid email":"Failed")}setAuthBusy(false)};
  const doEmailSignin=async()=>{setAuthErr("");if(!authEmail.trim())return setAuthErr("Enter email");if(!authPass)return setAuthErr("Enter password");setAuthBusy(true);try{await signInWithEmailAndPassword(auth,authEmail,authPass)}catch(e){setAuthErr(e.code==="auth/invalid-credential"?"Incorrect email/password":e.code==="auth/too-many-requests"?"Too many attempts":"Failed")}setAuthBusy(false)};
  const doForgot=async()=>{setAuthErr("");if(!authEmail.trim())return setAuthErr("Enter email");setAuthBusy(true);try{await sendPasswordResetEmail(auth,authEmail);sh("📧 Reset email sent!");setAuthMode("signin")}catch{setAuthErr("Failed")}setAuthBusy(false)};
  ...
  const addComment=async(qid,qObj)=>{if(!cmt.trim())return;const c={n:uName,ini:uIni,txt:cmt,tm:getIST().toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",hour12:true}),uid:au.uid};const comments=[...(qObj.comments||[]),c];await fbSet("quizzes",qid,{comments});setQuizzes(p=>p.map(q=>q.id===qid?{...q,comments}:q));setCmt("")};
  ...
  const saveContent=async(type)=>{const d=edForm.data;if(!d.title&&!d.t){sh("Title required");return}if(edForm.editing){await fbSet(type,d.id,d);sh("Updated!")}else{await fbAdd(type,{...d,order:Date.now()});sh("Created!")}setEdForm(null);loadData()};
  const deleteContent=async(type,id,name)=>{if(!confirm(`Delete "${name}"?`))return;await fbDel(type,id);sh("Deleted");loadData()};
  const postForum=async()=>{if(!fpT.trim())return;await fbAdd("forum",{author:uName,ini:uIni,uid:au.uid,title:fpT,cat:fpC,body:fpB,likes:0,replies:0,date:ds(getIST())});setFpT("");setFpB("");setNewForum(false);sh("Posted!");loadData()};
  ...
  if(scr==="login")return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#f8f7f4,#fdf6e3 40%,#e1f5ee 70%,#f8f7f4)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"system-ui"}}>
      <Logo size={100}/>
      <h1 style={{fontSize:"2.8rem",fontWeight:300,color:T.txt,marginTop:8,letterSpacing:6,fontFamily:"Georgia,serif"}}>SKINARIO</h1>
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"system-ui",color:T.txt}}>
      <div style={{position:"sticky",top:0,zIndex:100,background:"#ffffffee",backdropFilter:"blur(16px)",borderBottom:"1px solid "+T.border,padding:"6px 24px"}}>
        <div style={{maxWidth:W,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>go("home")}>
            <Logo size={36}/>
            <div><div style={{fontSize:"1.15rem",fontWeight:300,color:T.txt,letterSpacing:4,fontFamily:"Georgia,serif"}}>SKINARIO</div></div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:2}}>
            ...
            {uPhoto?<img src={uPhoto} onClick={()=>go("me")} style={{width:32,height:32,borderRadius:"50%",border:"2px solid "+T.tealBg,marginLeft:8,cursor:"pointer"}}/>:<div onClick={()=>go("me")} style={{...T.av(32,T.tealBg,T.teal),marginLeft:8,cursor:"pointer"}}>{uIni}</div>}
          </div>
        </div>
      </div>
      ...
      {pg==="home"&&!selA&&<div>
        <div style={{...T.card,borderLeft:"3px solid "+T.gold,background:"linear-gradient(135deg,#fff,#fdfcf8)",padding:24}}>
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:14}}>
            ...
            <div><h2 style={{fontSize:"1.4rem",fontWeight:700,margin:0}}>Welcome, {uName.split(" ")[0]} 👋</h2><p style={{color:T.txt2,fontSize:".9rem",marginTop:3}}>Daily quizzes, expert insights & community.</p></div>
          </div>
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}><button onClick={()=>go("quiz")} style={T.btn}>🧠 Today's quiz</button><button onClick={()=>go("forum")} style={T.btnO}>💬 Discussion</button><button onClick={()=>go("library")} style={T.btnO}>📚 Resources</button></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,margin:"16px 0"}}>
          {[["🧠",totA,"Quizzes"],["✅",acc+"%","Accuracy"],["📚",resources.length,"Resources"],["🎥",videos.length,"Videos"]].map(([i,v,l])=>
            <div key={l} style={{...T.card,textAlign:"center",padding:"14px 6px",marginBottom:0}}><div style={{fontSize:"1.1rem"}}>{i}</div><div style={{fontSize:"1.3rem",fontWeight:700,color:T.teal}}>{v}</div><div style={{fontSize:".58rem",color:T.mute,textTransform:"uppercase",marginTop:2}}>{l}</div></div>)}
        </div>
        <h3 style={{fontSize:"1.05rem",fontWeight:700,marginBottom:12}}>Latest articles</h3>
        {articles.length===0&&<p style={{color:T.mute}}>No articles yet. Admins can create them from the Admin panel.</p>}
      ...
      {pg==="quiz"&&<div>
        ...
        {ld&&<div style={{...T.card,textAlign:"center",padding:50}}><p style={{color:T.mute}}>⏳ Generating clinical question...</p></div>}
        {!ld&&!qObj&&<div style={{...T.card,textAlign:"center",padding:40}}>{selD===today?<><div style={{fontSize:"2rem",marginBottom:10}}>🔬</div><p style={{color:T.teal,fontSize:"1.05rem",fontWeight:600}}>Today's question</p><p style={{color:T.mute,fontSize:".88rem",margin:"8px 0 16px"}}>10:00 AM IST daily</p>{isAdm&&<button onClick={genQuiz} style={T.btn}>🤖 Generate now</button>}</>:<p style={{color:T.mute}}>No question for this date</p>}</div>}
        {!ld&&qObj&&<div style={{display:"grid",gridTemplateColumns:"1fr 340px",gap:16,alignItems:"start"}}>
          ...
          <div style={T.card}><div style={{fontSize:".88rem",color:T.teal,fontWeight:600,marginBottom:10}}>💬 Discussion ({qObj.comments?.length||0})</div>
            <div style={{maxHeight:380,overflowY:"auto"}}>{(qObj.comments||[]).map((x,i)=><div key={i} style={{padding:"8px 0",borderBottom:"1px solid "+T.border}}><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><div style={T.av(20,T.tealBg,T.teal)}>{x.ini}</div><b style={{fontSize:".78rem"}}>{x.n}</b><span style={{fontSize:".6rem",color:T.mute}}>{x.tm}</span></div><div style={{fontSize:".82rem",color:T.txt2,paddingLeft:26,lineHeight:1.5}}>{x.txt}</div></div>)}{!qObj.comments?.length&&<p style={{color:T.mute,fontSize:".8rem"}}>No comments yet.</p>}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
          {resources.map(r=><div key={r.id} style={{...T.card,display:"flex",gap:14,alignItems:"center",marginBottom:0}}>
            ...
            <div style={{flex:1}}><h4 style={{fontSize:".9rem",fontWeight:600,lineHeight:1.3}}>{r.title||r.t}</h4><div style={{fontSize:".72rem",color:T.mute,marginTop:3}}>{r.pages?r.pages+"p · ":""}{r.size||""}</div>
              <div style={{marginTop:6}}>{r.free||isPd?<button onClick={()=>r.url?window.open(r.url,"_blank"):sh("No URL yet")} style={{...T.btn,...T.btnSm}}>📥 Download</button>:<button style={{...T.btnO,...T.btnSm,color:T.gold,borderColor:T.gold}}>🔒 Premium</button>}</div></div>
          </div>)}
        </div>
        {!isPd&&resources.some(r=>!r.free)&&<div style={{...T.card,textAlign:"center",marginTop:16,borderLeft:"3px solid "+T.gold,padding:24}}><h4 style={{color:T.teal,fontWeight:700}}>🔓 Unlock premium</h4><button style={{...T.btnGold,marginTop:10}}>₹999/month</button></div>}
      </div>}
      ...
      {pg==="forum"&&<div>
        ...
        {newForum&&<div style={{...T.card,borderLeft:"3px solid "+T.teal}}><input value={fpT} onChange={e=>setFpT(e.target.value)} placeholder="Title..." style={{...T.inp,marginBottom:10}}/><select value={fpC} onChange={e=>setFpC(e.target.value)} style={{...T.inp,marginBottom:10}}>{TOPICS.map(t=><option key={t} value={t}>{t}</option>)}</select><textarea value={fpB} onChange={e=>setFpB(e.target.value)} placeholder="Details..." rows={3} style={{...T.txa,marginBottom:10}}/><button onClick={postForum} style={T.btn}>Publish</button></div>}
        {forumPosts.length===0&&!newForum&&<p style={{color:T.mute}}>No posts yet.</p>}
        {forumPosts.map(p=><div key={p.id} style={T.card}><div style={{display:"flex",gap:12}}><div style={T.av(36,T.tealBg,T.teal)}>{p.ini||"?"}</div><div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between"}}><b style={{fontSize:".88rem"}}>{p.author}</b><span style={{fontSize:".65rem",color:T.mute}}>{fD(p.date)}</span></div><span style={{...T.tag(T.tealBg,T.teal),marginTop:4}}>{p.cat}</span><h4 style={{fontSize:"1rem",fontWeight:600,marginTop:6,lineHeight:1.4}}>{p.title}</h4><p style={{fontSize:".88rem",color:T.txt2,marginTop:4,lineHeight:1.6}}>{p.body}</p><div style={{fontSize:".75rem",color:T.mute,marginTop:8}}>❤️ {p.likes||0} · 💬 {p.replies||0}</div></div></div></div>)}
      </div>}

      {/* RANK */}
      {pg==="rank"&&<div style={{...T.card,maxWidth:640}}><h3 style={{fontSize:"1.15rem",fontWeight:700,marginBottom:14}}>🏆 Leaderboard</h3>
        {leaderboard.length===0&&<p style={{color:T.mute}}>No scores yet. Answer quizzes to appear!</p>}
        {leaderboard.map((u,i)=>{const uAcc=u.totalAnswered?Math.round(u.totalCorrect/u.totalAnswered*100):0;const isMe=u.id===au?.uid;
          return<div key={u.id} style={{display:"flex",alignItems:"center",gap:12,padding:12,borderRadius:12,marginBottom:6,background:isMe?T.tealBg:"#fff",border:`1px solid ${isMe?T.teal:T.border}`}}>
            ...
            {u.photo?<img src={u.photo} style={{width:36,height:36,borderRadius:"50%",border:"2px solid "+(isMe?T.teal:T.border)}}/>:<div style={T.av(36,isMe?T.teal:T.tealBg,isMe?"#fff":T.teal)}>{u.initials||"?"}</div>}
      ...
      {pg==="admin"&&isAdm&&<div>
        ...
        {aTab==="stats"&&<div style={T.card}><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>{[["Articles",articles.length],["Resources",resources.length],["Videos",videos.length],["Forum",forumPosts.length],["Quizzes",quizzes.length],["Users",allUsers.length]].map(([l,v])=><div key={l} style={{textAlign:"center",padding:14,background:T.bg,borderRadius:10}}><div style={{fontSize:"1.4rem",fontWeight:700,color:T.teal}}>{v}</div><div style={{fontSize:".6rem",color:T.mute,textTransform:"uppercase"}}>{l}</div></div>)}</div></div>}

        {aTab==="quiz"&&<div style={T.card}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><span style={{color:T.mute}}>{quizzes.length} questions</span><button onClick={genQuiz} style={T.btn}>🤖 Generate today</button></div>
        ...
        {aTab==="articles"&&<div style={T.card}>{edForm?.type==="articles"?<AdminForm type="Article" fields={[["title","Title"],["cat","Category","select"],["author","Author"],["date","Date (YYYY-MM-DD)"],["body","Content","textarea"],["feat","Featured","check"]]} onSave={()=>saveContent("articles")}/>
          ...
          {articles.map(a=><div key={a.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid "+T.border}}><div><div style={{fontWeight:500,fontSize:".88rem"}}>{a.title}</div><div style={{fontSize:".72rem",color:T.mute}}>{fD(a.date)} · {a.cat}</div></div><div style={{display:"flex",gap:4}}><button onClick={()=>setEdForm({type:"articles",data:{...a},editing:true})} style={{...T.btnO,...T.btnSm}}>Edit</button><button onClick={()=>deleteContent("articles",a.id,a.title)} style={T.btnDanger}>Del</button></div></div>)}</>}</div>}

        {aTab==="resources"&&<div style={T.card}>{edForm?.type==="resources"?<AdminForm type="Resource" fields={[["title","Title"],["url","Download URL"],["pages","Pages"],["size","Size"],["icon","Emoji icon"],["free","Free","check"]]} onSave={()=>saveContent("resources")}/>
      ...
      <div style={{textAlign:"center",padding:"22px 0",borderTop:"1px solid "+T.border,marginTop:20}}>
        <Logo size={28}/>
        <div style={{fontSize:".65rem",color:T.light,letterSpacing:2,textTransform:"uppercase",marginTop:6}}>SKINARIO · <span style={{color:T.gold,fontWeight:600}}>By Absolute Institute</span></div>
      </div>
